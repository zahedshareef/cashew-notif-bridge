...
class RulesActivity : AppCompatActivity() {
    ...
    private fun showEditDialog(rule: NotificationRule?) {
        val dialog = RuleEditDialogFragment.newInstance(rule)
        dialog.show(supportFragmentManager, "edit_rule")
    }
...
class RulesAdapter(
    ...
    inner class RuleViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(rule: NotificationRule) {
            ...
            chipIncome.visibility = if (rule.isIncome) View.VISIBLE else View.GONE

            chipEnabled.setOnCheckedChangeListener { _, checked ->
